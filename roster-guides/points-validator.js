(function(root){
  ...
  const enhancementName=value=>String(value||'').replace(/\s*\([^)]*\)\s*$/,'').replace(/\s+[-–—]\s+\d+\s*pts?\s*$/i,'').trim();

  function check(roster,faction){
    const catalog=root.WH_POINTS_CATALOG?.[faction],unresolved=[],occurrences=new Map();let total=0;
    ...
    for(const raw of roster.enhancements||[]){
      ...
      if(!enhancement)unresolved.push(`Enhancement: ${name}`);else total+=Number(enhancement.value);
    }
    return{total,unresolved,difference:total-Number(roster.declared||0)};
  }
