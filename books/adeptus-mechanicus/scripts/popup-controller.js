    ...
    createCard(id,index){
      ...
      if(related.length){
        ...
        related.forEach((relatedId,relatedIndex)=>{
          ...
          const button=document.createElement('button');button.type='button';button.className='term-link';button.dataset.term=relatedId;button.textContent=this.terms[relatedId].title;paragraph.append(button);
        });
