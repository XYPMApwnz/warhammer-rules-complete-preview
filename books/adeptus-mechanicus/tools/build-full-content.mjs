...
const unitCard=unit=>{
  ...
  return `<article class="unit-card surface${unit.status==='Warhammer Legends'?' legends-card':''}" id="${unit.id}" data-track="${unit.id}" data-unit-title="${esc(unit.title)}" data-keywords="${esc(unit.keywords.join('|'))}"><div class="unit-header"><div><div class="eyebrow">${esc(unit.status)}</div><h3>${esc(unit.title)}</h3></div><div class="unit-status">${unit.status==='Warhammer Legends'?'LEGENDS':points?`${esc(points)} PTS`:'CODEX'}</div></div><div class="local-nav">${tabs}<button class="local-tab related-rules-trigger" type="button">Stratagems &amp; Enhancements</button></div><section class="unit-part" id="${parts.profile}"><h4>Profile & Weapons</h4>${stats(unit)}${weapons(unit)}</section><section class="unit-part" id="${parts.abilities}"><h4>Abilities</h4><div class="ability-list">${abilities}</div></section><section class="unit-part" id="${parts.composition}"><h4>Composition & Wargear</h4><p>${decorate(unit.composition,unit.id)}</p>${gear}</section><section class="unit-part" id="${parts.keywords}"><h4>Keywords</h4><div class="keyword-list">${unit.keywords.map(x=>`<span>${esc(x)}</span>`).join('')}</div></section>${provenance}</article>`;
};
...
<section class="hero section surface am-hero" id="start" data-track="start"><div class="hero-content"><div class="eyebrow">11th Edition // 10 Detachments // ${rules.datasheets.length} Datasheets</div><h1>Flesh fails.<br><em>The Machine endures.</em></h1><p>Adeptus Mechanicus rules reference with current points, local search, roster filtering and official Faction Pack replacements.</p><div class="source">Official Faction Pack v1.0 · current Codex layer from pinned 11e community data</div></div><div class="hero-mark"><img src="./assets/mechanicus-logo.png" width="512" height="512" alt="Adeptus Mechanicus emblem"><span>THE OMNISSIAH KNOWS ALL</span></div></section>
