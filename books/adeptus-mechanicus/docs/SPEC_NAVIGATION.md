...
Контракт поведения перенесён из Death Guard Rules v5. Числа разделов и имена источников в разделе 10 адаптированы к текущему прототипу Mechanicus; взаимодействия, геометрические допуски и критерии приёмки сохранены без ослаблений.
...
- Контрольная линия scroll-spy и безопасный отступ обычного перехода используют одно значение `18px` ниже фиксированного header и общей sticky-панели поиска; отдельные несинхронизированные константы запрещены.
- Фактическая высота общей sticky-панели `readerTools` одинаково добавляется в scroll-spy и destination всех целей. Изменение её высоты обновляет кеш геометрии через `ResizeObserver`.
...
- Дерево и тело документа должны собираться из единого источника `content/adeptus-mechanicus-prototype.en.json`; расхождение дерева и тела блокируется QA-проверкой соответствия `data-nav-target`, `id` и `data-track`.
- Текущая конфигурация прототипа содержит 36 глобальных целей: Start, Updates, Core Rules, 5 detachments с тремя дочерними частями каждый, 4 datasheet-цели и 2 группы Glossary.
...
- Каждый detachment имеет три отдельные вертикальные цели: `Detachment Rule`, `Enhancements`, `Stratagems`; все пять заголовков `Stratagems` видимы в теле.
