...
- Каждый из десяти вложенных пунктов Glossary после клика получает собственный `aria-current="location"` и не оставляет активным предыдущего соседа.
...
- Полная конфигурация содержит 104 глобальные цели: 6 корневых пунктов; 1 дочерний пункт Updates; 1 группа Core Rules с 5 дочерними правилами; 9 detachment-групп с 27 дочерними частями; 9 групп Datasheets с 36 unit-целями; 10 групп Glossary. Итого: `6 + 1 + 1 + 5 + 9 + 27 + 9 + 36 + 10 = 104`.
...
- Каждый detachment имеет три отдельные вертикальные цели. В дереве используются подписи `Detachment Rule`, `Enhancement`, `Stratagems`; подпись `Enhancement` ведёт к общему блоку с видимым заголовком `Enhancements`. Все девять заголовков `Stratagems` видимы в теле.
