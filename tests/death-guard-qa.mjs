...
check('Mega Glossary transitions persist a return record',read('scripts/full-entry-controller.js').includes('wh40k-mega-glossary-return')&&read('scripts/app.js').includes('wh40k-mega-glossary-return'));
...
check('all five style layers are linked',cssFiles.every(file=>html.includes('href="./'+file+(file==='styles/content.css'?'?v=17':file==='styles/popups.css'?'?v=15':file==='styles/navigation.css'?'?v=10':'?v=9')+'"')));
...
check('safe mobile controls activate on touch-down',navigation.includes("this.menuButton.addEventListener('touchstart'")&&popups.includes("close.addEventListener('touchstart'")&&contentCss.includes('[data-term]:active'));
check('touch terms open on pointer-down and roll back cancelled scroll gestures',popups.includes("document.addEventListener('pointerdown'")&&popups.includes("document.addEventListener('pointercancel'")&&popups.includes('this.touchTerm={trigger,pointerId:event.pointerId')&&popups.includes('this.ids=this.touchTerm.ids')&&popups.includes('this.touchTerm?.trigger===trigger'));
...
check('book scripts and styles use the current release token',[...cssFiles.filter(file=>!['styles/content.css','styles/navigation.css','styles/popups.css'].includes(file)),...files.filter(file=>!['scripts/navigation-controller.js','scripts/popup-controller.js','scripts/full-entry-controller.js','scripts/journey-controller.js','scripts/ui-controllers.js','scripts/app.js'].includes(file))].every(file=>html.includes('./'+file+'?v=9'))&&html.includes('./styles/content.css?v=17')&&html.includes('./styles/navigation.css?v=10')&&html.includes('./styles/popups.css?v=15')&&html.includes('./scripts/navigation-controller.js?v=14')&&html.includes('./scripts/popup-controller.js?v=20')&&html.includes('./scripts/full-entry-controller.js?v=5')&&html.includes('./scripts/journey-controller.js?v=11')&&html.includes('./scripts/ui-controllers.js?v=11')&&html.includes('./scripts/app.js?v=17'));
