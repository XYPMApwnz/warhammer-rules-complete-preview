...
check('touch terms open on pointer release without waiting for click',popups.includes("document.addEventListener('pointerup'")&&popups.includes("event.pointerType!=='touch'")&&popups.includes('this.touchTerm={trigger,until:performance.now()+750}')&&popups.includes('this.touchTerm?.trigger===trigger'));
