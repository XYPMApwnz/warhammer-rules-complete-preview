...
check('Roster Guides preserve the storage contract',rosterGuidesApp.includes("const STORAGE_KEY='wh40k-rosters-v1'")&&rosterGuidesApp.includes('function rosterId(text)')&&rosterGuidesApp.includes("'death guard':'../books/death-guard/reader.html'")&&rosterGuides.includes('app.js?v=5')&&sw.includes('./roster-guides/app.js?v=5'));
...
check('Roster Guides compare current Army Book points without blocking save',rosterGuides.includes('points-data.js?v=2')&&rosterGuides.includes('points-validator.js?v=2')&&rosterGuidesApp.includes('window.WHRosterPoints.check')&&rosterGuidesApp.includes('Points warning:')&&rosterGuidesApp.includes('The roster was still saved.')&&!rosterGuidesApp.includes('Roster is ready to build.'));
check('Roster Guides and Death Guard share one source parser',rosterGuides.includes('roster-parser.js?v=1')&&read('books/death-guard/reader.html').includes('roster-parser.js?v=1')&&read('books/death-guard/scripts/roster-filter.js').includes('WHRosterParser.parse'));
check('owned Enhancements are derived without mutating Army Book data',read('books/death-guard/reader.html').includes('roster-enhancements.js?v=1')&&read('books/shared/roster-enhancements.js').includes("item.effect === 'furnace'")&&read('books/shared/roster-enhancements.js').includes("item.effect === 'critical-hit-5'"));
