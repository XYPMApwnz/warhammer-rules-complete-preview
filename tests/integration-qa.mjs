...
for(const slug of generatedArmyBooks){
  const html=read(`books/${slug}/reader.html`),app=read(`books/${slug}/scripts/app.js`),mobile=read(`books/${slug}/mobile/index.html`),bookCss=read(`books/${slug}/styles/book.css`),context=JSON.parse(read(`glossary/contexts/${slug}.json`));
  ...
  check(`${slug} exposes Library, Glossary and Related Rules`,html.includes('href="../../index.html"')&&html.includes('href="../../glossary/index.html"')&&html.includes('../shared/army-related-rules.js?v=3'));
  check(`${slug} exposes the forced Phone view contract`,app.includes('readerPath')&&mobile.includes('searchParams.set("view","mobile")')&&bookCss.includes('html[data-view="mobile"] .nav-menu')&&bookCss.includes('html[data-view="mobile"] .unit-card.ds-layout .ds-main-grid'));
