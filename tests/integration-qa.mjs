...
const books={
  'death-guard':{version:'9',reader:'reader.html',versions:{'styles/tokens.css':'10','styles/navigation.css':'11','styles/content.css':'24','styles/popups.css':'15','scripts/roster-filter.js':'12','scripts/navigation-controller.js':'15','scripts/popup-controller.js':'22','scripts/full-entry-controller.js':'8','scripts/journey-controller.js':'11','scripts/ui-controllers.js':'11','scripts/related-rules.js':'6','scripts/app.js':'27'},app:'scripts/app.js',usesPopupGlossary:true,files:['assets/icon-v4.svg','styles/tokens.css','styles/layout.css','styles/navigation.css','styles/content.css','styles/popups.css','scripts/roster-filter.js','scripts/navigation-controller.js','scripts/popup-controller.js','scripts/full-entry-controller.js','scripts/journey-controller.js','scripts/ui-controllers.js','scripts/related-rules.js','scripts/app.js']},
  'core-rules':{version:null,reader:'reader/index.html',rootPrefix:'../../../',libraryEntry:'reader/index.html',versions:{'app.js':'2','reader/styles.css':'5','reader/app.js':'9'},app:'app.js',usesPopupGlossary:false,files:['styles.css','config.js','basic-content.js','app.js','content/core-rules.source.en.js','content/core-rules.en.js','reader/index.html','reader/search-index.json','reader/styles.css','reader/app.js']},
  'adeptus-mechanicus':{version:'13',versions:{'styles/content.css':'14','styles/popups.css':'15','scripts/popup-controller.js':'19','scripts/ui-controllers.js':'14','scripts/app.js':'18'},app:'scripts/app.js',usesPopupGlossary:true,files:['assets/mechanicus-logo.png','styles/tokens.css','styles/layout.css','styles/navigation.css','styles/content.css','styles/popups.css','styles/mechanicus.css','scripts/data.js','scripts/navigation-controller.js','scripts/popup-controller.js','scripts/journey-controller.js','scripts/ui-controllers.js','scripts/app.js']}
};

for(const file of ['service-worker.js','glossary-return.js','books/shared/navigation-targets.js','books/shared/datasheet-layout.js','books/shared/popup-content.js','books/shared/roster-entities.js',...Object.entries(books).map(([slug,book])=>`books/${slug}/${book.app}`)]){
...
check('Roster Guides preserve the storage contract',rosterGuidesApp.includes("const STORAGE_KEY='wh40k-rosters-v1'")&&rosterGuidesApp.includes('function rosterId(text)')&&rosterGuidesApp.includes("'death guard':'../books/death-guard/reader.html'")&&rosterGuides.includes('app.js?v=4')&&sw.includes('./roster-guides/app.js?v=4'));
...
check('missing roster and faction states are explicit',rosterGuidesApp.includes("alert('Saved roster not found.')")&&rosterGuidesApp.includes('<h2>Faction not found</h2>')&&rosterGuidesApp.includes("values('FACTION KEYWORD')[0]||''"));
...
check('Mechanicus records have no fake reader action',rosterGuides.includes('Adeptus Mechanicus rosters can be saved')&&rosterGuidesApp.includes('disabled>Reader unavailable</button>'));
check('roster storage key remains compatible',rosterGuidesApp.includes("const STORAGE_KEY='wh40k-rosters-v1'"));
check('Roster Guides compare current Army Book points without blocking save',rosterGuides.includes('points-data.js?v=1')&&rosterGuides.includes('points-validator.js?v=1')&&rosterGuidesApp.includes('window.WHRosterPoints.check')&&rosterGuidesApp.includes('Points warning:')&&rosterGuidesApp.includes('The roster was still saved.')&&!rosterGuidesApp.includes('Roster is ready to build.'));
...
check('Every result search prioritises title matches',read('books/core-rules/reader/app.js').includes('normalizeSearch(a.title).includes(query)')&&read('books/core-rules/app.js').includes('!at.includes(query)')&&read('glossary/viewer.js').includes('!a.title.en.toLowerCase().includes(query)')&&read('books/adeptus-mechanicus/scripts/ui-controllers.js').includes("card.style.order=title.includes(query)?'-1':''")&&read('books/adeptus-mechanicus/scripts/ui-controllers.js').includes('!a.title.toLocaleLowerCase().includes(query)'));
...
check('Mega Glossary return UI is shared, versioned and precached',read('glossary/index.html').includes('id="libraryBack"')&&read('glossary/index.html').includes('../glossary-return.js?v=1')&&read('glossary/index.html').includes('viewer.js?v=6')&&sw.includes('"./glossary-return.js?v=1"')&&sw.includes('"./glossary/viewer.js?v=6"')&&sw.includes('"./glossary/viewer-popup.css?v=1"'));
